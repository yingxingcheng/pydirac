...
class Outupt(MSONable):
    ...
    def parse(self):
        ...
        if not self.inp_settings:
            self.input = Input(self.inp_settings)
    ...
    def parse_orbit(self, e_min=-20.0, e_max=25.0):
        ...
        self.atom_info = Atom.from_file(self.filename)
        if self.atom_info:
...
class Input(MSONable):
    ...
    def as_dict(self) -> dict:
        return jsanitize(self.settings, strict=True)
...
class Mol(MSONable):
    ...
    def as_dict(self) -> dict:
        return jsanitize(self.setting, strict=True)
